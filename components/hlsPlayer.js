...
export default class HlsPlayer extends Component {
  ...
  constructor(props) {
    ...
    this.hls = undefined;
    this.transcodingId = undefined; // transcoding id from manifest
    ...
    this.contentServer = new ContentServer(this.server);
   ...
   */
  getResolutions() {
    const promise = this.type === 'movie' ? this.contentServer.getMovieResolutions(this.id) : this.contentServer.getEpisodeResolutions(this.id);
    return promise.then(({ resolutions, directplay }) => {
        const result = [];
        if (directplay) {
          result.push(new Resolution(resolutions.length, 'Directplay'));
        }
        for (let i = resolutions.length - 1; i >= 0; i--) {
          result.push(new Resolution(i, resolutions[i]));
        }
        return result;
      });
  }
  ...
  getLanguages() {
    return new Promise(resolve => {
      const promise = this.type === 'movie' ? this.contentServer.getMovieLanguages(this.id) : this.contentServer.getEpisodeLanguages(this.id);
      promise.then(languages => {
        const defaultLanguage = this.getDefaultLanguage(languages);
        this.setState({
          audioLanguages: languages,
          activeLanguageStreamIndex: defaultLanguage != null ? defaultLanguage.stream : 0
        }, () => {
          resolve();
        });
      });
   ...
   */
  getTranscodingIdFromManifest(data) {
    if (data.levels.length > 0 && data.levels[0].url.length > 0) {
      const regex = /transcoding=([^&]+)/i;
      try {
  ...
  onManifestParsed(event, data) {
    this.transcodingId = this.getTranscodingIdFromManifest(data);
   ...
   */
  getSubtitles() {
    const promise = this.type === 'movie' ? this.contentServer.getMovieSubtitles(this.id) : this.contentServer.getEpisodeSubtitles(this.id);
    promise.then(subtitles => this.setState({ directplaySubtitles: subtitles }));
  }
   ...
   */
  stopTranscoding() {
    if (this.transcodingId !== undefined) {
      this.contentServer.stopTranscoding(this.transcodingId);
    }
    this.transcodingId = undefined;
  }
  ...
  updateVideoProgress() {
    ...
    if (!isNaN(currentTime)) {
      this.contentServer.updateVideoProgress(this.type, this.id, currentTime);
    }
  }
