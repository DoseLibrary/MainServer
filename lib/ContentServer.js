...
export default class ContentServer {
    ...
    search(query) {
        return new Promise((resolve, reject) => {
            validateServerAccess(this.#server, token => {
                const url = `${this.#ip}/api/search?query=${query}`;
    ...
    */
    ...
    pingTranscoding(transcodingId) {
        return new Promise(resolve => {
            validateServerAccess(this.#server, token => {
                const url = `${this.#ip}/api/video/hls/ping?transcoding=${transcodingId}`;
                resolve(this.#requestNoResponse(url, this.getStandardPostOptions(token)));
            })
        });
    }
}