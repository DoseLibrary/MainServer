...
export async function buildApp(config: AppConfig) {
  ...
  await registerApiRoutes(app, new AuthService(database), config.NODE_ENV === 'production', undefined, new ScanCoordinator(database, config), new CatalogService(database), join(config.CONFIG_PATH, 'images'), config.NODE_ENV === 'development' && isEmbeddedDatabase(config.DATABASE_URL), plugins, pluginScheduler);
