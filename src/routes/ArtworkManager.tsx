...
export function ArtworkManager({ open, itemId, onOpenChange, onApplied }: ArtworkManagerProps) {
  ...
  async function choose(change: { posterPath?: string; backdropPath?: string }, label: string) {
    setBusy(true); setNotice(undefined);
    try { await api.setArtwork(itemId, change); setNotice(`${label} updated`); onApplied?.(); }
    ...
    <Modal open={open} onOpenChange={onOpenChange}>
      <ModalContent aria-describedby="artwork-manager-description" className="max-w-3xl">
        <ModalHeader>
          ...
          <ModalDescription id="artwork-manager-description">Pick a poster or backdrop from the provider. The chosen image is cached locally.</ModalDescription>
        </ModalHeader>
        {error ? <div role="alert" className="rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm text-destructive">{error}</div>
          ...
          : (options.posters.length === 0 && options.backdrops.length === 0) ? <p className="py-8 text-center text-muted-foreground">No provider artwork is available for this title.</p>
          : (
                ...
                <section aria-labelledby="artwork-posters">
                  ...
                  <ul className="grid list-none grid-cols-3 gap-3 p-0 sm:grid-cols-4 md:grid-cols-6">
                    {options.posters.map((option) => (
                      <li key={option.path}>
                        <button type="button" disabled={busy} onClick={() => void choose({ posterPath: option.path }, 'Poster')} className="block w-full overflow-hidden rounded-md ring-1 ring-border transition-shadow hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50">
                ...
                <section aria-labelledby="artwork-backdrops">
                  ...
                  <ul className="grid list-none grid-cols-2 gap-3 p-0 sm:grid-cols-3">
                    {options.backdrops.map((option) => (
                      <li key={option.path}>
                        <button type="button" disabled={busy} onClick={() => void choose({ backdropPath: option.path }, 'Backdrop')} className="block w-full overflow-hidden rounded-md ring-1 ring-border transition-shadow hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50">
